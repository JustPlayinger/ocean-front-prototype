...
| 按视野取数（v1.10） | 一天可有多片数据：作业海域**主片**（0.05°）+ 按当前视野取的概览/明细片（每片自带 `grid` 与 `resolutionDeg`，最多 4 片、主片永不淘汰）；渲染按「粗 → 细」叠加（球面档只画概览片，0.05° 在球面上≈0.3 像素却要重投影上万条游程）；视野跑出作业海域时地图顶部明说「图上数据按该窗口取，右侧结论仍基于作业海域」 | e2e：离线只有 1 片且被标主片、作业海域范围=主片网格、像元查询回报分辨率、降采样档位 1/4/20/40、离线不设视野窗口；线上探针：球面自动取全球 2° 概览（band/cold/warm/缺测各 1 条路径）、东亚 60° 取 0.2°、黑潮 8° 取 0.05°（39 个对象）、北大西洋 8° 取 0.05°（61 个对象）、drawMap 9–16 ms、0 运行期错误 |
