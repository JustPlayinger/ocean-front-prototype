...
- **两级海温（v1.11）**：另有可选字段 `sst_coarse`（结构与 `sst` 完全一致），来源是
  `raw/sst_global/`（全球**粗格**，1° = `--stride 20`，单日 ≈0.3MB）。前端把它**垫在** `sst`（东海 0.05°）
  下面：于是"作业海域很细、别处也有海温"，而不是二选一。抓取：
  `tools/pipeline/sst_global_pipeline.py --stride 20 --years 2024`（后台常驻、可续跑）。
