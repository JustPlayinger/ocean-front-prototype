...
- **当前排期（`tools/pipeline/sst_plan.sh`，v2 · 含上游探测与自愈）**：**先全球、后东海**——
  B 全球 1°：2024+2023 → C 全球 1°：2022+2021 → D 全球 0.25°：2024+2023（≈5GB）→
  A 东海明细 0.05° 续跑；0.05° 全分辨率那行默认注释（两年≈80–100GB，需先扩盘到 200GB 级）。
  自愈规则：**阶段没产出就等上游恢复再重试该阶段**（最多 24 次 × 10 分钟），不会"失败一次就跑完队列"；
